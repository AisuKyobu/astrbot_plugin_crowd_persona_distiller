...
- **vision 支持**：蒸馏/回复可附带聊天截图，提升多模态场景下的画像质量
- **WebUI 管理**：
  - 群友列表（按群筛选 · 关键字搜索 · 消息数/最后发言排序 · 蒸馏状态统计）
  - 人格编辑器（保存 · 重新蒸馏 · 增量更新 · 修正人格 · 删除，按危险度分组）
  - 称呼映射表（多别名 · 搜索 · 增删改查）
  - 按群配置（扮演模式 · @触发开关 · 改名片开关）
  - 数据导入（qq-chat-exporter JSON · 预览勾选 · 进度条）
...
3. **蒸馏人格**  
   WebUI → 群友列表 → 点击「蒸馏」按钮，或勾选多个群友后「蒸馏选中」批量排队  
   需先配置 `distill_provider`（LLM 提供商）  
   消息数需 ≥ `min_distill_messages`（默认 50 条）  
   蒸馏为后台任务执行，面板实时反馈进度，页面刷新不影响后台进行  
   蒸馏时自动合并该用户的群聊 + 私聊消息
...
| Tab | 功能 |
|-----|------|
| 群友列表 | 按群筛选 → 关键字搜索（名字/别名/QQ号） → 消息数/最后发言排序 → 4 格统计条（已蒸馏/可蒸馏/未达标/合计）→ 卡片按状态分层 → 可勾选批量蒸馏（后台任务排队） |
| 人格弹窗 | Markdown 编辑器 · 保存（Ctrl+S）· 重新蒸馏 · 增量更新 · 修正人格 · 删除（按钮按危险度分组，删除右对齐）· sticky 状态条反馈 · 未保存修改关闭/覆盖前确认 |
| 称呼管理 | QQ号 → 称呼映射表，支持多别名（半角逗号分隔），表格内搜索，增删改查 |
| 群配置 | 按群设置扮演模式（随机/指定/关闭）、@触发开关、改名片开关 |
| 数据导入 | 上传 qq-chat-exporter JSON（群聊/私聊），自动识别聊天类型，预览勾选后带进度条导入 |

> 视觉基线：design token + 三层阴影、统一缓动、SVG 图标系统、状态色语义化，遵循 `prefers-reduced-motion`。
